...
(function(window, undefined) {
	...
	jQuery.fn = jQuery.prototype = {
		...
		init: function(selector, context, rootjQuery) {
			...
			if (typeof selector === "string") {
				if (selector.charAt(0) === "<" && selector.charAt(selector.length - 1) === ">" && selector
					.length >= 3) {
	...
	jQuery.support = (function() {
		...
		jQuery(function() {
			...
			if (window.getComputedStyle) {
				...
				support.reliableMarginRight = !parseFloat((window.getComputedStyle(marginDiv,
					null) || {}).marginRight);
			}
	...
	jQuery.extend({
		...
		attr: function(elem, name, value) {
			...
			if (value !== undefined) {
				...
				} else if (hooks && notxml && "set" in hooks && (ret = hooks.set(elem, value, name)) !==
					undefined) {
					return ret;
	...
	if (!getSetAttribute) {
		...
		nodeHook = jQuery.valHooks.button = {
			get: function(elem, name) {
				var ret = elem.getAttributeNode(name);
				return ret && (name === "id" || name === "name" || name === "coords" ? ret.value !== "" :
						ret.specified) ?
					ret.value :
					undefined;
			},
			...
			while (t--) {
				...
				while (j--) {
					...
					if ((mappedTypes || origType === handleObj.origType) &&
						...
						(!selector || selector === handleObj.selector || selector === "**" && handleObj
							.selector)) {
						handlers.splice(j, 1);
	...
	if (!jQuery.support.changeBubbles) {
		...
		jQuery.event.special.change = {
			...
			handle: function(event) {
				...
				if (this !== elem || event.isSimulated || event.isTrigger || (elem.type !== "radio" && elem
						.type !== "checkbox")) {
		...
		off: function(types, selector, fn) {
			...
			if (types && types.preventDefault && types.handleObj) {
				...
				jQuery(types.delegateTarget).off(
					handleObj.namespace ? handleObj.origType + "." + handleObj.namespace : handleObj
					.origType,
					handleObj.selector,
					handleObj.handler
				);
	...
	(function(window, undefined) {
		...
		setDocument = Sizzle.setDocument = function(node) {
			...
			if (support.getIdNotName) {
				...
				Expr.find["ID"] = function(id, context) {
					if (typeof context.getElementById !== strundefined && !documentIsXML) {
						...
						return m ?
							m.id === id || typeof m.getAttributeNode !== strundefined && m
							.getAttributeNode("id").value === id ? [m] :
							undefined : [];
					}
				};
				Expr.filter["ID"] = function(id) {
					var attrId = id.replace(runescape, funescape);
					return function(elem) {
						var node = typeof elem.getAttributeNode !== strundefined && elem
							.getAttributeNode("id");
			...
			sortOrder = docElem.compareDocumentPosition ?
				function(a, b) {
					...
					if ((compare = b.compareDocumentPosition && a.compareDocumentPosition && a
				...
				function(a, b) {
					...
					if (a === b) {
						...
						return (~b.sourceIndex || MAX_NEGATIVE) - (contains(preferredDoc, a) && ~a
							.sourceIndex || MAX_NEGATIVE);
		...
		Sizzle.matchesSelector = function(elem, expr) {
			...
			if (support.matchesSelector && !documentIsXML && (!rbuggyMatches || !rbuggyMatches.test(
				expr)) && !rbuggyQSA.test(expr)) {
				try {
				...
				"ATTR": function(name, operator, check) {
					return function(elem) {
						...
						return operator === "=" ? result === check :
							...
							operator === "$=" ? check && result.substr(result.length - check
							.length) === check :
							operator === "~=" ? (" " + result + " ").indexOf(check) > -1 :
							operator === "|=" ? result === check || result.substr(0, check.length +
								1) === check + "-" :
							false;
					};
				},
			...
			pseudos: {
				...
				"text": function(elem) {
					...
					return elem.nodeName.toLowerCase() === "input" &&
						elem.type === "text" &&
						((attr = elem.getAttribute("type")) == null || attr.toLowerCase() === elem
						.type);
				},
			...
			var matcherCachedRuns = 0,
				...
				superMatcher = function(seed, context, xml, results, expandContext) {
					var elem, j, matcher,
						...
						elems = seed || byElement && Expr.find["TAG"]("*", expandContext && context
							.parentNode || context),
	...
	jQuery.extend({
		...
		dir: function(elem, dir, until) {
			...
			while (cur && cur.nodeType !== 9 && (until === undefined || cur.nodeType !== 1 || !jQuery(
					cur).is(until))) {
				if (cur.nodeType === 1) {
					matched.push(cur);
				}
						...
						for (i = 0; i < hasScripts; i++) {
							...
							if (rscriptType.test(node.type || "") &&
								...
								if (node.src) {
									...
									jQuery.globalEval((node.text || node.textContent || node
	...
	jQuery.extend({
		clone: function(elem, dataAndEvents, deepDataAndEvents) {
			...
			if (jQuery.support.html5Clone || jQuery.isXMLDoc(elem) || !rnoshimcache.test("<" + elem
					.nodeName + ">")) {
				...
				if (elem || elem === 0) {
					...
					if (jQuery.type(elem) === "object") {
						...
						if (!jQuery.support.tbody) {
							...
							while (j--) {
								if (jQuery.nodeName((tbody = elem.childNodes[j]), "tbody") && !tbody
									.childNodes.length) {
	...
	if (jQuery.expr && jQuery.expr.filters) {
		jQuery.expr.filters.hidden = function(elem) {
			return (elem.offsetWidth === 0 && elem.offsetHeight === 0) || (!jQuery.support
				.reliableHiddenOffsets && ((elem.style && elem.style.display) || jQuery.css(elem,
					"display")) === "none");
		};
	...
	jQuery.extend({
		...
		ajax: function(url, options) {
			...
			if (s.beforeSend && (s.beforeSend.call(callbackContext, jqXHR, s) === false || state ===
				2)) {
	...
	jQuery.ajaxPrefilter("json jsonp", function(s, originalSettings, jqXHR) {
		...
		var callbackName, overwritten, responseContainer,
			jsonProp = s.jsonp !== false && (rjsonp.test(s.url) ?
				"url" :
				typeof s.data === "string" && !(s.contentType || "").indexOf(
...
$.extend({
	hello:function(){
		console.log('hello,'+(name?name:'大林子'))
	},
	show:function(){
		return 'color哈哈哈哈哈哈哈'
	},
